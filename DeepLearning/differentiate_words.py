...
for k in range(len(DIMENSIONS)):
    ...
    with open(
        os.path.join(DATA_DIR, "extreme_examples_{}.txt".format(DIMENSIONS[k][0])), "r", encoding='utf-8'
    ) as f:
        wordcount_a = collections.Counter(f.read().split())

    with open(
        os.path.join(DATA_DIR, "extreme_examples_{}.txt".format(DIMENSIONS[k][1])), "r", encoding='utf-8'
    ) as f:
    ...
    with open(
        os.path.join(DATA_DIR, "special_words_{}.txt".format(DIMENSIONS[k][0])), "w", encoding='utf-8'
    ) as f:
        for key in a.keys():
    ...
    with open(
        os.path.join(DATA_DIR, "special_words_{}.txt".format(DIMENSIONS[k][1])), "w", encoding='utf-8'
    ) as f:
        for key in b.keys():
            mod = regex.sub("", str(key))
            if mod not in WORDS_TO_REMOVE:
